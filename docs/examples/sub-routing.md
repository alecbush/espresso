...
This example demonstrates mounting multiple sub-applications under different path prefixes. Each sub-app defines its own routes relative to the mount point. Routes here simply return JSON messages to keep the focus on sub-routing rather than functional behavior.
